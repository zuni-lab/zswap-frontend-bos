...
const formattedZswapBalance =
  !zswapBalance || zswapBalance === "-"
    ? "-"
    : Big(zswapBalance).toFixed(5, BIG_ROUND_DOWN);
...
return (
  <Main>
    All tokens are here!!!
    <Widget
      src={`${config.ownerId}/widget/ZSwap.Data.Stake`}
      props={{ config, onLoad }}
    />
    ...
    <MyAccountContent>
      ...
      <MyAccountCardWrapper style={{ marginTop: "10px" }}>
        <div>
          <TokenValue>
            <div>{formattedZswapBalance}</div>
            <ZSWAPIcon />
          </TokenValue>
          <GrayContent>Your ZSwap Tokens</GrayContent>
        </div>
        <div style={{ width: "130px" }}>
          <Widget
            src={`${config.ownerId}/widget/ZSwap.Element.Button`}
        ...
        <MyAccountCardWrapper style={{ marginTop: "10px" }}>
          ...
          <RewardsFinishedTime>
            <Widget
              ...
              props={{
                message:
                  "Staking rewards are included in the ZSwap price. ZSwap price increases every epoch (12~15 hours).",
              }}
            />
