...
const formattedZswapBalance =
  zswapBalance === "-" ? "-" : Big(zswapBalance).toFixed(5, BIG_ROUND_DOWN);

const zswapPrice = Big(
  Near.view(config.contractId, "ft_price", `{}`) ?? "0"
).div(Big(10).pow(24));
const nearPriceInZSWAP = zswapPrice.eq(0)
  ? "1"
  : Big(1).div(zswapPrice).toFixed(5, BIG_ROUND_DOWN);

function getReceivedDelayedUnstakeNear() {
  const { unstakeMax, inputValue } = state;
  if (!isValid(zswapBalance) || !isValid(inputValue)) {
    return "-";
  }
  const delayedUnstakeZswap = unstakeMax ? zswapBalance : inputValue;
  const _delayedUnstakeNear = Big(delayedUnstakeZswap)
    .times(zswapPrice)
    .toFixed(5);
...
const onChange = (e) => {
  ...
  if (
    ...
    (isNaN(Number(unstakeAmount)) ||
      unstakeAmount === "" ||
      Big(unstakeAmount).lt(nearPriceInZSWAP) ||
      Big(unstakeAmount).gt(Big(zswapBalance)))
  ) {
    if (
      isNaN(Number(unstakeAmount)) ||
      unstakeAmount === "" ||
      ...
      State.update({
        ...
        inputError: `Stake at least ${nearPriceInZSWAP} ZSwap`,
      });
    } else {
      State.update({
        unstakeMax: false,
        onClickMax: false,
        inputValue: unstakeAmount,
        inputError: `Max is ${formattedZswapBalance} ZSwap`,
      });
    }
    return;
  }
...
const onClickMax = () => {
  if (
    isNaN(Number(zswapBalance)) ||
    zswapBalance === "" ||
    Big(zswapBalance).lt(nearPriceInZSWAP)
  ) {
    State.update({
      unstakeMax: true,
      inputValue: formattedZswapBalance,
      inputError: `Stake at least ${nearPriceInZSWAP} NEAR`,
    });
    return;
  } else {
    State.update({
...
const onClickUnstake = async () => {
  ...
  const amount = Big(inputValue)
    .times(zswapPrice)
    .times(Big(10).pow(ZSWAP_DECIMALS))
    .toFixed(0);
...
const TOKEN_ZSWAP = { id: config.contractId, decimals: ZSWAP_DECIMALS };
...
return (
  <StakeFormWrapper>
    ...
    <Widget
      ...
      props={{
        placeholder: "ZSwap amount to unstake",
        value: state.inputValue,
        onChange,
        onClickMax,
        inputError: state.inputError,
        balance: `${formattedZswapBalance} ZSwap`,
        iconName: "ZSwap",
        iconUrl:
    ...
    <UnstakeTabWrapper>
      ...
      >
        <UnstakeTabTitle>
          <p>INSTANT UNSTAKE</p>
          <Widget
            src={`${config.ownerId}/widget/ZSwap.Element.Tooltip`}
            props={{
              message:
                "The slippage and fee of instant unstake is based on the ZSwap/wNEAR StableSwap pool on Ref",
            }}
          />
        </UnstakeTabTitle>
        <EstimateGetValue>
    ...
    {state.showConfirmInstantUnstake && (
      <Widget
        src={`${config.ownerId}/widget/ZSwap.Page.Swap.Sell.Modal.ConfirmInstantUnstake`}
        props={{
          config,
    ...
    {state.showConfirmDelayedUnstake && (
      <Widget
        src={`${config.ownerId}/widget/ZSwap.Page.Swap.Sell.Modal.ConfirmDelayedUnstake`}
        props={{
          config,
    ...
    {state.showGoToWithdraw && (
      <Widget
        src={`${config.ownerId}/widget/ZSwap.Page.Swap.Sell.Modal.WithdrawNow`}
        props={{
          config,
          onClickWithdraw: () => {
            State.update({
